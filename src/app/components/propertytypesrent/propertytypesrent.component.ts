...
export class PropertytypesrentComponent {
  ...
  fetchPropertyTypeBuysIn() {
    ...
    if (type && city) {
      this.http.get<any>(`${environment.apiUrl}propertytypesrentin/${type}/${city}?page=${this.currentPage}`).subscribe(
          (response) => {
            const oldScrollY = window.scrollY;
  
            this.propertytype = this.propertytype || [];
            this.propertytype = [...this.propertytype, ...(response.responseData?.propertytypesrentin?.data || []),];
            this.setMetaTags(
              response.meta.title,
              response.meta.description,
            );
  
            this.lastPage = response.responseData?.propertytypesrentin?.last_page;
            this.propertytypecount = response.responseData?.propertytypesrentin?.total;
    ...
    this.isDropdownOpen = false;
      switch (option) {
        case 'Price - Low to High':
          this.propertytype = this.propertytype.sort((a: any, b: any) => this.sortByPrice(a, b));
          break;
        case 'Price - High to Low':
          this.propertytype = this.propertytype.sort((a: any, b: any) => this.sortByPrice(b, a));
          break;
        case 'Most Recent':
          this.propertytype = this.propertytype.sort((a: any, b: any) => this.sortByRecent(a, b));
          break;
        case 'Relevance':
  ...
  filterByPrice(minPrice: number, maxPrice: number): void {
    this.filteredData = this.propertytype.filter((property: { minprice: string; maxprice: string; }) => {
